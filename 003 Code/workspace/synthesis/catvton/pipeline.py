...
from catvton.attn_processor import SkipAttnProcessor
from catvton.utils_model import get_trainable_module, init_adapter
...
class CatVTONPipeline:
    def __init__(
        ...
        self.noise_scheduler = DPMSolverMultistepScheduler.from_pretrained(base_ckpt, subfolder="scheduler", algorithm_type="dpmsolver++", use_karras_sigmas=True)
        ...
        with tqdm.tqdm(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if hasattr(self, "progress_callback") and self.progress_callback is not None:
                    self.progress_callback(i + 1, latents.clone())
                #########################################################################
